...
const traefikBin = process.env.TRAEFIK_BIN || 'traefik';
const logDir = '/var/log';
const managerLog = fs.createWriteStream(path.join(logDir, 'manager.log'), { flags: 'a' });
...
function spawnTraefik(): ChildProcess {
  ...
  const tlog = fs.createWriteStream(path.join(logDir, 'traefik.log'), { flags: 'a' });
...
const serverLogs: Record<Slot, fs.WriteStream> = {
  server1: fs.createWriteStream(path.join(logDir, 'server1.log'), { flags: 'a' }),
  server2: fs.createWriteStream(path.join(logDir, 'server2.log'), { flags: 'a' })
};
...
function spawnServer(slot: Slot): ChildProcess {
  ...
  const proc = spawn('npm', ['start'], { cwd, stdio: ['ignore', 'pipe', 'pipe'] });
...
app.get('/status', async (_req, res) => {
  const [h1, h2] = await Promise.all([checkHealth('server1'), checkHealth('server2')]);
  res.json({
    server1: h1,
    server2: h2,
    active,
